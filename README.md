...
| `cancel` | — | 取消在途任务：带 `task_id` = 精确取消；省略 = **全部取消**（语义见表格下方） |
| `shutdown` | — | 回 `"bye"` 后优雅退出（见 1.5） |

**取消语义**（`cancel` 的 `task_id` 用法有两档，写错不报错，只是取消不掉）：

- **精确取消**：带上要取消的那个任务的 `task_id`。任务收到取消后，其接口调用
  会尽快以 `canceled` 报错结束；已经在写盘的那一张图会先写完，不会留下半个文件
  （落盘是原子的，见第 5 节）。宿主会收到**两帧**：先是 cancel 命令自己的响应
  `{task_id:N,"result":{"canceled":1}}`（`canceled:0` 表示任务已经自己跑完了，
  没找到可取消的），随后目标任务以 `{task_id:N,"error_code":"canceled"}` 报错
  结束 —— 宿主桥应该在第一帧就把该任务的 Promise 转成取消拒绝，第二帧按
  迟到响应丢弃（`pending` 已删，自然接不住）。
- **全部取消**：**省略 `task_id`（或显式 `null`）**。⚠️ 发数字 `0` **不是**
  全取消 —— 零值判据是"省略或 null"，`0` 会被当成普通 id 去精确匹配（永远
  查不到，返回 `canceled:0`）。另外全取消的响应帧**不带 `task_id` 字段**
  （请求没带则响应不回显，见 1.3），形如 `{"result":{"canceled":3}}`，
  宿主桥不能走按 `task_id` 匹配的常规路由，要按帧形状专门识别。

**取消的错误码是 `canceled`，优先于一切业务码。** 取消打断的在途环节
（比如一条进行中的 HTTP 请求）会让服务层就地产生 `not_found` / `network`
之类的业务错误，但那些只是取消的副作用；宿主看到的 `error_code` 始终是
`canceled`，不要按业务码分支。
...
```jsonc
{
  ...
  "cache_ttl_days": 0,          // 渲染素材缓存保留天数；0 = 永不清理（默认）
...
`.cache` 里的内容全部是**可重新下载的派生数据**，删掉没有任何不可恢复的损失。
默认永不清理（每次渲染先查缓存、没有才下载）；在意增长的宿主可以设
`cache_ttl_days`（如 `30`），进程**启动时**会把修改时间早于该天数的文件删掉 ——
选在启动时做是因为此刻尚无渲染在跑，删除不会与在途请求竞争。
